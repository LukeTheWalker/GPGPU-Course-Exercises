...
int main (int argc, char * argv[]){
    ...
    int ntails = ngroups;
	if (ngroups > 1) ntails = round_mul_up(ngroups, 4);

    int *h_a, *d_in, *d_out, *d_tails;
    float t1;
    cudaError_t err;
    ...
    scan_sliding_window<<<ngroups, lws, lws*sizeof(int)>>>((int4*)d_in, (int4*)d_out, d_tails, round_div_up(nels, 4), 32);
    ...
    if (ngroups > 1){
        scan_fixup<<<ngroups, lws>>>((int4*)d_out, d_tails, round_div_up(nels, 4), 32);
    }
